...
-----
Comment:
This script is used to analyze the results of the model, 
...
def get_visualization(test_data, model):
    ...
    grayscale_cam = cam(input_tensor=video[0].unsqueeze(dim=0), aug_smooth=True, eigen_smooth=True)
...
def main(hparams):
    ...
    # save the results 
    torch.save(pred, Path("/workspace/skeleton/misc") / f"{hparams.model.model}_{sampler}_{fold}_pred.pt" )
    torch.save(label, Path("/workspace/skeleton/misc") / f"{hparams.model.model}_{sampler}_{fold}_label.pt" )

    logging.info(f"save the pred and label into {Path('/workspace/skeleton/misc')} / {hparams.model.model}_{sampler}_{fold}_pred.pt and {hparams.model.model}_{sampler}_{fold}_label.pt")
