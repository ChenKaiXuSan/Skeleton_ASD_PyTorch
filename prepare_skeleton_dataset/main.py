...
# -*- coding:utf-8 -*-
'''
File: /workspace/skeleton/prepare_skeleton_dataset/ntu_pose_extraction.py
...
-----
Last Modified: Wednesday June 12th 2024 4:14:17 pm
...
args.det_config = '/workspace/skeleton/prepare_skeleton_dataset/faster-rcnn_r50-caffe_fpn_ms-1x_coco-person.py'  # noqa: E501
...
args.pose_config = '/workspace/skeleton/prepare_skeleton_dataset/td-hm_hrnet-w32_8xb64-210e_coco-256x192_infer.py'  # noqa: E501
...
def ntu_det_postproc(vid, det_results):
    ...
    for may_dis in vid.split('/')[-1].split('_'):
        try :
            label = label_mapping[may_dis]
            break
        except:
            continue
...
@hydra.main(version_base=None,
    ...
    config_name="prepare_skeleton_dataset.yaml",)
def main(config):
    
    video_path = Path(config.gait_dataset.data_path)
    output_path = Path(config.gait_dataset.save_path)

    args.device = config.device
    args.video = video_path
    args.skip_postproc = config.skip_postproc

    for train_flag in video_path.iterdir():
        for one_class in train_flag.iterdir():
            for one_video_path in one_class.iterdir():

                anno = ntu_pose_extraction(one_video_path, config.skip_postproc)

    mmengine.dump(anno, output_path / f'{train_flag.name}.pkl')
